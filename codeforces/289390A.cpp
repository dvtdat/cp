...
public:
    ...
    Node* last;
    Node* next;
    Node(int data, Node* next = nullptr) : data(data), next(next), last(this), size(1) {};
};
...
bool get(int a, int b, const vector<int> &par)
{
    return (par[a] == par[b]);
}
...
void solve()
{
    ...
    for (int i = 0; i < q; ++i)
    {
        int u, v;
        cin >> order >> u >> v;
        if (order == "union") unite(u, v, par, list);
        else cout << (get(u, v, par) ? "YES" : "NO") << '\n';
    }
}
